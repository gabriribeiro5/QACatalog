...
def validate(validation_column, df):
    ufd_function = F.udf(lambda item: validate_total_installments(item), BooleanType())
...
def validate_total_installments(total_installments):
    try:    
        if int(total_installments) >= 1 | len(str(total_installments)) <= 3:
            return True
    except:
        return False
...
if __name__ == '__main__':
    ...
    columns = ['id', 'status', 'total_installments']
    df = spark.createDataFrame(data=data, schema = columns)

    print('Tabela de entrada:')
    df.show()

    df_ok, df_nok = validate('total_installments', df)
