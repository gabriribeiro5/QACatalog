...
if __name__ == "__main__":
    ...
    data = [
        (1, 'OK', "123456789qwertytestedeidcomatecinquentacaracteres"),
        (2, 'OK', "testedeidcomatecinquentacaracteres"),
        (3, 'NOK', "123456789qwertytestedeidcomatecinquentacaracteresmuitoalemdoesperado"),
        (4, 'NOK', 123456),
    ]

    columns = ['id', 'status', 'arrangment_id']
    df = spark.createDataFrame(data=data, schema = columns)
    print('Tabela de entrada:')
    df.show()

    df_ok, df_nok = validate('arrangment_id', df)
