...
def validate(validation_column, df):
    ufd_function = F.udf(lambda item: validate_arrangement_status(item), BooleanType())
...
def validate_arrangement_status(arrangement_status):
    try:    
        if int(arrangement_status) in range(1, 6):
            return True
    except:
        return False
...
if __name__ == '__main__':
    ...
    columns = ['id', 'status', 'arrangement_status']
    df = spark.createDataFrame(data=data, schema = columns)

    print('Tabela de entrada:')
    df.show()

    df_ok, df_nok = validate('arrangement_status', df)
