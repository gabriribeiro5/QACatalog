#=============================================================================#
#                                                                             #
# Objetivo: Script PySpark que atualiza o datamart arrangements-installments. #
...
#------------------------------- Descrição -----------------------------------#
#                                                                             #
# Lê os dados da raw zone e atualiza o datamart arrangements-installments na  #
#  refined zone.                                                              #
#                                                                             #
#------------------------------- Parâmetros ----------------------------------#
#                                                                             #
#>>> UPDATE_INSTALLMENTS = Referência no S3 da trasient zone de onde os dados #
#  serão atualizados.                                                         #
#>>> ARRANGEMENTS_UPDATE_DATAMART = Referência no S3 da transient zone onde   #
# os dados serão atualizados.                                                 #
#>>> DATAMART_INSTALLMENTS = Referência no S3 onde os dados do datamart de    #
# ARRANGEMENTS-INSTALLMENTS estão inseridos.                                  #
#                                                                             #
#=============================================================================#
...
args = getResolvedOptions(sys.argv, ['JOB_NAME', 
                                     ...
                                     'PROCESS_TYPE',
                                     'UPDATE_INSTALLMENTS',
                                     'DATAMART_INSTALLMENTS',])

job_name                     = args['JOB_NAME']
job_run_id                   = args['JOB_RUN_ID']
process_type                 = args['PROCESS_TYPE']
log_path                     = args['LOG_PATH']
ftcrm_installments           = args['UPDATE_ARRANGEMENTS']
datamart_installments        = args['DATAMART_INSTALLMENTS']


#Parâmetros principais
MAX_REGISTERS_REPARTITION = 250000

id_col = "installmentid"
update_column_list = [
        "installmentid",
        "installmentnumber",
        "installmentdate",
...
# Ajustando e adicionando colunas para o Datamart
updates_origem = updates_origem.withColumnRenamed("_installmentstatus", "_installmentstatusid") \
            .withColumn("_installmentstatus", 
                        F.when(F.col("_installmentstatusid") == 61000, "Em Aberto") \
                        .when(F.col("_installmentstatusid") == 61001, "Pago") \
                        .when(F.col("_installmentstatusid") == 61002, "Quebrado") \
                        .when(F.col("_installmentstatusid") == 61005, "Parcialmente Pago") \
                        .when(F.col("_installmentstatusid") == 61009, "Remessa Pendente"))
...
update_partition_paths = [datamart_installments + "year=" + x._year + "/month=" + x._month + "/day=" + x._day + "/" for x in update_partitions]
...
datamart_original = read_data_from_lake(datamart_installments, datamart_installments)
...
updates_datamart = updates_datamart.drop(*update_column_list + ["installmentstatusid"])
